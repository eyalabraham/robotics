...
/*
   ;	       DELAY_;
   ;	
	mov	word ptr DGROUP:_d,0     - 3+5+2•T  - min 8 cycle
@2@6:
	inc	word ptr DGROUP:_d       - 3+15+4•T - min 18 cycle
	cmp	word ptr DGROUP:_d,1000  - 3+10+2•T - min 13 cycle
	jb	short @2@6                  - 15/8     - min 15 cycle
                                              ------------------
                                               min 50 cycles rounded up
                                               bus cycle 250nSec
                                               1 mSec ~ 80 loop iterations
                                             500uSec  ~ 40 loop iterations
   ;	
*/

// ~500uSec delay
#define   DELAY_500uSEC    {                                \
                            unsigned int _d_;               \
                            for (_d_ = 0; _d_ < 40; _d_++); \
                           }

// ~250uSec delay
#define   DELAY_250uSEC    {                                \
                            unsigned int _d_;               \
                            for (_d_ = 0; _d_ < 20; _d_++); \
                           }


// ~100uSec delay
#define   DELAY_100uSEC    {                                \
                            unsigned int _d_;               \
                            for (_d_ = 0; _d_ < 8; _d_++);  \
                           }
